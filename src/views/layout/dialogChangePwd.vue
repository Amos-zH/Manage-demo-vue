<template>
    <el-dialog title="修改密码" :visible.sync="dialogVisible" width="20%" class="change-pwd">
        <ValidationObserver ref="pwdObserver" v-slot="{ passes }">
            <el-form ref="pwdForm" :model="pwdForm" label-width="auto">
                <ValidationProvider rules="required|alpha_num|min:6|max:6" name="原密码" v-slot="{ errors }">
                    <el-form-item :error="errors[0]" label="原密码" prop="oldPwd" inline-message>
                        <el-input
                ...
                <ValidationProvider rules="required|alpha_num|min:6|max:6" name="新密码" vid="newPwd" v-slot="{ errors }">
                ...
                <ValidationProvider rules="required|alpha_num|min:6|max:6|confirmed:newPwd,新密码" name="重复密码" v-slot="{ errors }">
